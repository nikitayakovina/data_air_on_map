...
export class LeafletMapsComponent implements AfterViewInit {
  ...
  private initMap(): void {
    this.locationService.getPositions().then(result => {
              ...
              Object.keys(sensor).forEach(_sensor => {
                if (_sensor === 'list') {
                  Object.keys(sensor[_sensor]).forEach(__sensor => {
                    ...
                    L.geoJSON(newPolygon).bindTooltip(`Индекс загрязненности воздуха: ${sensor[_sensor][__sensor].main.aqi.toString()}`).addTo(this._map);
                  });
                }
              });
            });
      ...
      // L.geoJSON(json).addTo(this._map);
    });
